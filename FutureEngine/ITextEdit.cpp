...
void ITextEdit::OnMouseMove(glm::vec2 position, glm::vec2 delta) {
    // Update selection if mouse button is pressed
    if (m_HasFocus && GameInput::Buttons[MOUSE_BUTTON_LEFT] && m_SelectionStart >= 0) {
...
// Modify HandleKeyInput to use the key repeat mechanism
