...
public:
    MessageBox(std::string message, std::function<void()> onClose = nullptr);
    ~MessageBox() = default;
...
MessageBox::MessageBox(std::string message, std::function<void()> onClose)
    : IWindow("Message", glm::vec2(0, 0), glm::vec2(400, 200)), // Initial size, will be adjusted
    m_OnClose(onClose)
{
    ...
    std::vector<std::string> lines = SplitText(message, maxWidth - 40); // Account for padding
